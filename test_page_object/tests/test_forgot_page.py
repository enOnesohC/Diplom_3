...
class TestForgotPage:
    ...
    @allure.description("Заходим на главную страницу, переходим на страницу авторизации, проверяем функцию восстановления пароля")
    def test_recover_password(self, browser_driver):
        ...
        with allure.step('Переходим на страницу авторизации, проверяем функцию восстановления пароля'):
    ...
    @allure.description("Переход по кнопке Забыли пароль")
    ...
    @allure.description("Поле Email при клике должно подсвечиваться")
    def test_hide_password(self, browser_driver):
        ...
        with allure.step('Поиск элемента, получение его класса и сравнение с эталоном'):
            ...
            tab = element.find_element(*LoginPageLocators.PARENT_OBJECT)
            tab_class = tab.get_attribute("class")
        assert tab_class == Data.tab_class
